          ...
          <div className="relative z-10">
            {encuestas.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-gray-400 mb-6">Registra la primera encuesta para comenzar el seguimiento</p>
                <button
                  onClick={() => setIsActualizacionModalOpen(true)}
                  className="inline-flex items-center gap-2 px-6 py-3 bg-blue-500/10 border border-blue-500/30 rounded-lg text-blue-500 hover:bg-blue-500/20 transition-all duration-300"
                >
                  <Plus className="w-5 h-5" />
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-primary/20">
                      ...
                      <th className="text-left p-4 text-gray-400 font-medium">Registrado</th>
                      <th className="text-left p-4 text-gray-400 font-medium">Candidatos</th>
                      <th className="text-center p-4 text-gray-400 font-medium">Acciones</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-primary/10">
                    {encuestas.map((encuesta) => (
                      <tr key={encuesta.id}>
                        <td className="p-4 text-white">{encuesta.nombre}</td>
                        <td className="p-4 text-gray-300">{encuesta.fecha}</td>
                        <td className="p-4 text-gray-300">{encuesta.empresa}</td>
                        <td className="p-4 text-gray-400">{formatFechaCreacion(encuesta.fechaCreacion)}</td>
                          ...
                          <ul>
                            {encuesta.candidatos.map((candidato) => (
                              <li key={candidato.id}>
                                ...
                                {': '}
                                <span>{candidato.intencionVoto}%</span>
                              </li>
                            ))}
                          </ul>
                        </td>
                        <td className="p-4 text-center">
                          ...
                          >
                            <svg className="w-4 h-4 text-red-400 group-hover:text-red-300" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6M1 7h22M10 3h4a1 1 0 011 1v2H9V4a1 1 0 011-1z"></path></svg>
                          </button>
                        </td>
                      </tr>
                    ))}
